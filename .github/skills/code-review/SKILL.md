...
| マイグレーションとDBテスト | マイグレーションを追加・変更したPRで、対応する DBテスト（`supabase/tests/database/`）が追加・更新されているか（新しいテーブルなら本人/他人/admin/anon × 参照/追加/変更/削除、`00_schema_security.test.sql` の `tables_are` の一覧）、`plan(N)` の件数が合っているか、RLS の対象テーブルをサブクエリで引いて「他人の行」を特定していないか（RLS で空になりテストが素通りする）、適用済みのマイグレーションファイルを変更していないか | `.github/instructions/supabase.instructions.md`「マイグレーション運用」「DBテスト（`supabase/tests/`）」 |
